...
<STYLE type=text/css>.STYLE1 {
  ...
  HashMap map = dao.select("select * from cgdinfo where id="+id).get(0);
  %>
  <form action="/hysupermarketjxcsys/hysupermarketjxcsys?ac=cgdedit&id=<%=id %>" name="f1" method="post" onsubmit="return ck()" >
<TABLE cellSpacing=1 cellPadding=3 width="95%" align=center bgColor=#6ab6b6 
border=0>
  <TBODY>
  <TR>
    <TD  height=24>【编辑采购单】</TD>
  ...
  <TR align=center>
    ...
    <TD class=forumrow  height=24 align="left">
    	<select id="pid" name="pid">
    		...
    		for(HashMap map0:list){%>
    		<option value="<%=map0.get("id") %>"><%=map0.get("pname") %></option>
    		<%} %>
    	</select>
    </TD>
  </TR>
  <TR align=center>
    ...
    <TD class=forumrow  height=24 align="left">
    	<select id="suppliersid" name="suppliersid">
    		...
    		<option value="<%=map1.get("id") %>" <%if(map1.get("id").equals(map.get("suppliersid"))){out.print("selected==selected");} %>><%=map1.get("sname") %></option>
    		<%} %>
    	</select>
    </TD>
  </TR>
