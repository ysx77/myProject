...
<STYLE type=text/css>.STYLE1 {
  ...
  <body>
<form action="admin/kcmanage.jsp?f=f" method="post">
	<%
	 String sql = "select * from product where 1=1 and delstatus='0' ";
	 String url = "/hysupermarketjxcsys/admin/kcmanage.jsp?1=1";
...
border=0>
  <TBODY>
  <TR>
    <TD  height=24>【商品库存管理】</TD>
  	<TD  height=24 align="right" colspan="7"><a href="admin/productadd.jsp" target=frmright></a></TD>
  </TR>
  <TR align=center>
    ...
    <TD class=forumrow  height=24>库存数量</TD>
  </TR>
  <%
   ...
   int warnningnum = 0;//库存预警值
   HashMap wmap = dao.select("select * from kcwarnning where id=1").get(0);
   warnningnum = Integer.parseInt(wmap.get("num").toString());
  ...
  <TR align=center>
    ...
    <%  int znum = Info.getkc(map.get("id").toString());%>
    <%=znum %>
    <%if(znum<warnningnum){%>
    	<font color="red">(库存量低于预警值)</font>
    <%} %>
    </TD>
  </TR>
  <%} %>
